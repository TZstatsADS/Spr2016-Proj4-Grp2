...
get_movie_info<-function(i){
  ...
  movie1<-tryCatch(read_html(paste("http://www.amazon.com/exec/obidos/ASIN/", ASIN.inq, sep="")))
  movie1.title=
    movie1 %>% 
    html_node("title") %>%
    html_text()
  ...
  if(!is.null(omdb.entry$imdbID[1])){
    movie.info<-find_by_id(omdb.entry$imdbID[1], include_tomatoes=F)

    genre<-get_genres(movie.info)
    country<-get_countries(movie.info)
  
  
    result<-list(id=ASIN.inq,title=omdb.entry$Title[1],genre=genre, country=country)
  }else
  
  {result<-list(id=ASIN.inq,title=NULL,genre=NULL, country=NULL)}
